...
enum HoldState {
  ...
  localAndRemote(
    PjsipConnectFlutter.kHoldStateLocalAndRemote,
    "LocalAndRemote",
  );
  ...
  static HoldState from(int val) {
    switch (val) {
      case PjsipConnectFlutter.kHoldStateLocal:
        return HoldState.local;
      case PjsipConnectFlutter.kHoldStateRemote:
        return HoldState.remote;
      case PjsipConnectFlutter.kHoldStateLocalAndRemote:
        return HoldState.localAndRemote;
      default:
...
enum PlayerState {
  ...
  static PlayerState from(int val) {
    switch (val) {
      case PjsipConnectFlutter.kPlayerStateStarted:
        return PlayerState.started;
      case PjsipConnectFlutter.kPlayerStateStopped:
        return PlayerState.stoppped;
      default:
...
class CallModel extends ChangeNotifier implements IPjsipConnectData {
  ...
  String get nameAndExt =>
      displName.isEmpty ? remoteExt : "$displName ($remoteExt)";
  ...
  bool get isLocalHold =>
      (_holdState == HoldState.local) ||
      (_holdState == HoldState.localAndRemote);

  /// Returns true if call put on hold by remote side
  bool get isRemoteHold =>
      (_holdState == HoldState.remote) ||
      (_holdState == HoldState.localAndRemote);
  ...
  Future<void> playFile(String pathToMp3File, {bool loop = false}) async {
    _logs?.print(
      'Starting play file callId:$myCallId $pathToMp3File loop:$loop',
    );
    try {
      _playerId =
          await PjsipConnectFlutter().playFile(myCallId, pathToMp3File, loop) ??
          0;
    } on PlatformException catch (err) {
      _logs?.print(
        'Can\'t start playing file callId:$myCallId Err: ${err.code} ${err.message}',
      );
      return Future.error((err.message == null) ? err.code : err.message!);
    }
  }
  ...
  Future<void> recordFile(String pathToMp3File) async {
    ...
    try {
      ...
      _logs?.print(
        'Can\'t start recording file callId:$myCallId Err: ${err.code} ${err.message}',
      );
      return Future.error((err.message == null) ? err.code : err.message!);
    }
  }
  ...
  Future<String?> getSipHeader(String headerName) async {
    try {
      String? hdrVal = await PjsipConnectFlutter().getSipHeader(
        myCallId,
        headerName,
      );
      _logs?.print(
        'GetSipHeader of callId:$myCallId "$headerName" = "$hdrVal"',
      );
      return hdrVal;
    } on PlatformException catch (err) {
      _logs?.print(
        'Can\'t get getSipHeader callId:$myCallId Err: ${err.code} ${err.message}',
      );
      return "";
    }
  }
  ...
  Future<String?> getStats() async {
    try {
      ...
      _logs?.print(
        'Can\'t getStats callId:$myCallId Err: ${err.code} ${err.message}',
      );
      return "";
    }
  }
  ...
  void onHeld(HoldState holdState) {
    ...
    _state = (holdState == HoldState.none)
        ? CallState.connected
        : CallState.held;
    notifyListeners();
  }
...
class CallsModel extends ChangeNotifier
    with IterableMixin<CallModel>
    implements IPjsipConnectData {
  ...
  @protected
  List<CallModel> get callItems => _callItems;
  @protected
  IAccountsModel get accountsModel => _accountsModel;
  ...
  Future<void> invite(CallDestination dest) async {
    _logs?.print(
      'Trying to invite ${dest.toExt} from account:${dest.fromAccId}',
    );
    try {
  ...
  void onIncomingSip(
    ...
    _logs?.print(
      'onIncoming callId:$callId accId:$accId from:$hdrFrom to:$hdrTo withVideo:$withVideo',
    );
  ...
  void onConnected(int callId, String from, String to, bool withVideo) {
    _logs?.print(
      'onConnected callId:$callId from:$from to:$to withVideo:$withVideo',
    );
  ...
  void onTerminated(int callId, int statusCode) async {
    ...
    String reason =
        await PjsipConnectFlutter().getSipHeader(callId, "Reason") ?? "";
    _cdrs?.setTerminated(
      callId,
      statusCode,
      reason,
    ...
    if (_switchedCallId == callId) {
      _switchedCallId = _callItems.isNotEmpty
          ? _callItems.first.myCallId
          : kEmptyCallId;
    }
  ...
  void onVideoUpgraded(int callId, bool withVideo) {
    ...
    bool isUpgradeModeRecvOnly = _accountsModel.isUpgradeToVideoModeRecvOnly(
      call.accUri,
    );
  ...
  void onSyncCallsState(Map<String, dynamic> argsMap) {
    for (var c in argsMap['callsList'] ?? []) {
      CallModel? newCall = CallModel.fromJson(
