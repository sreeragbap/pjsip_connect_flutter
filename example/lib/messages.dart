...
class _MessagesListPagePageState extends State<MessagesListPage> {
    ...
    return Column(children: [
      Expanded(
        ...
        itemBuilder: (BuildContext context, int index) {
          return _messageListTileSelector(messages, index);
        },
  ...
  Widget _getMsgRowTrailing(int index) {
    return PopupMenuButton<MsgAction>(
        onSelected: (MsgAction action) {
          _onMsgMenuAction(action, index);
        },
  ...
  void _sendMessage() {
    ...
    //Send
    MessageDestination msgDest = MessageDestination(
        _phoneNumbCtrl.text, accounts.selAccountId!, _bodyTextCtrl.text);
    context
        .read<MessagesModel>()
        .send(msgDest)
        .then((_) => setState(() {
